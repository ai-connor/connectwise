...
type ApiPostTimeEntriesByIdDetachRequest struct {
	ctx context.Context
	ApiService *TimeEntriesAPIService
	id int32
	clientId *string
...
// 
func (r ApiPostTimeEntriesByIdDetachRequest) ClientId(clientId string) ApiPostTimeEntriesByIdDetachRequest {
	r.clientId = &clientId
	return r
}

func (r ApiPostTimeEntriesByIdDetachRequest) Execute() (*http.Response, error) {
	return r.ApiService.PostTimeEntriesByIdDetachExecute(r)
}

/*
PostTimeEntriesByIdDetach Detch TimeEntry

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param id entryId
 @return ApiPostTimeEntriesByIdDetachRequest
*/
func (a *TimeEntriesAPIService) PostTimeEntriesByIdDetach(ctx context.Context, id int32) ApiPostTimeEntriesByIdDetachRequest {
	return ApiPostTimeEntriesByIdDetachRequest{
		ApiService: a,
		ctx: ctx,
		id: id,
...
// Execute executes the request
func (a *TimeEntriesAPIService) PostTimeEntriesByIdDetachExecute(r ApiPostTimeEntriesByIdDetachRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "TimeEntriesAPIService.PostTimeEntriesByIdDetach")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/time/entries/{id}/detach"
	localVarPath = strings.Replace(localVarPath, "{"+"id"+"}", url.PathEscape(parameterValueToString(r.id, "id")), -1)
