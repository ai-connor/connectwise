/*
Connectwise Manage Public Endpoints

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2025.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package cwapi

import (
	"encoding/json"
)

// checks if the UnpostedProcurementTaxableLevel type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UnpostedProcurementTaxableLevel{}

// UnpostedProcurementTaxableLevel struct for UnpostedProcurementTaxableLevel
type UnpostedProcurementTaxableLevel struct {
	Id *int32 `json:"id,omitempty"`
	TaxLevel *int32 `json:"taxLevel,omitempty"`
	TaxCodeXref *string `json:"taxCodeXref,omitempty"`
	TaxAmount NullableFloat64 `json:"taxAmount,omitempty"`
	Info *map[string]string `json:"_info,omitempty"`
}

// NewUnpostedProcurementTaxableLevel instantiates a new UnpostedProcurementTaxableLevel object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUnpostedProcurementTaxableLevel() *UnpostedProcurementTaxableLevel {
	this := UnpostedProcurementTaxableLevel{}
	return &this
}

// NewUnpostedProcurementTaxableLevelWithDefaults instantiates a new UnpostedProcurementTaxableLevel object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUnpostedProcurementTaxableLevelWithDefaults() *UnpostedProcurementTaxableLevel {
	this := UnpostedProcurementTaxableLevel{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *UnpostedProcurementTaxableLevel) GetId() int32 {
	if o == nil || IsNil(o.Id) {
		var ret int32
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UnpostedProcurementTaxableLevel) GetIdOk() (*int32, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *UnpostedProcurementTaxableLevel) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int32 and assigns it to the Id field.
func (o *UnpostedProcurementTaxableLevel) SetId(v int32) {
	o.Id = &v
}

// GetTaxLevel returns the TaxLevel field value if set, zero value otherwise.
func (o *UnpostedProcurementTaxableLevel) GetTaxLevel() int32 {
	if o == nil || IsNil(o.TaxLevel) {
		var ret int32
		return ret
	}
	return *o.TaxLevel
}

// GetTaxLevelOk returns a tuple with the TaxLevel field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UnpostedProcurementTaxableLevel) GetTaxLevelOk() (*int32, bool) {
	if o == nil || IsNil(o.TaxLevel) {
		return nil, false
	}
	return o.TaxLevel, true
}

// HasTaxLevel returns a boolean if a field has been set.
func (o *UnpostedProcurementTaxableLevel) HasTaxLevel() bool {
	if o != nil && !IsNil(o.TaxLevel) {
		return true
	}

	return false
}

// SetTaxLevel gets a reference to the given int32 and assigns it to the TaxLevel field.
func (o *UnpostedProcurementTaxableLevel) SetTaxLevel(v int32) {
	o.TaxLevel = &v
}

// GetTaxCodeXref returns the TaxCodeXref field value if set, zero value otherwise.
func (o *UnpostedProcurementTaxableLevel) GetTaxCodeXref() string {
	if o == nil || IsNil(o.TaxCodeXref) {
		var ret string
		return ret
	}
	return *o.TaxCodeXref
}

// GetTaxCodeXrefOk returns a tuple with the TaxCodeXref field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UnpostedProcurementTaxableLevel) GetTaxCodeXrefOk() (*string, bool) {
	if o == nil || IsNil(o.TaxCodeXref) {
		return nil, false
	}
	return o.TaxCodeXref, true
}

// HasTaxCodeXref returns a boolean if a field has been set.
func (o *UnpostedProcurementTaxableLevel) HasTaxCodeXref() bool {
	if o != nil && !IsNil(o.TaxCodeXref) {
		return true
	}

	return false
}

// SetTaxCodeXref gets a reference to the given string and assigns it to the TaxCodeXref field.
func (o *UnpostedProcurementTaxableLevel) SetTaxCodeXref(v string) {
	o.TaxCodeXref = &v
}

// GetTaxAmount returns the TaxAmount field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *UnpostedProcurementTaxableLevel) GetTaxAmount() float64 {
	if o == nil || IsNil(o.TaxAmount.Get()) {
		var ret float64
		return ret
	}
	return *o.TaxAmount.Get()
}

// GetTaxAmountOk returns a tuple with the TaxAmount field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *UnpostedProcurementTaxableLevel) GetTaxAmountOk() (*float64, bool) {
	if o == nil {
		return nil, false
	}
	return o.TaxAmount.Get(), o.TaxAmount.IsSet()
}

// HasTaxAmount returns a boolean if a field has been set.
func (o *UnpostedProcurementTaxableLevel) HasTaxAmount() bool {
	if o != nil && o.TaxAmount.IsSet() {
		return true
	}

	return false
}

// SetTaxAmount gets a reference to the given NullableFloat64 and assigns it to the TaxAmount field.
func (o *UnpostedProcurementTaxableLevel) SetTaxAmount(v float64) {
	o.TaxAmount.Set(&v)
}
// SetTaxAmountNil sets the value for TaxAmount to be an explicit nil
func (o *UnpostedProcurementTaxableLevel) SetTaxAmountNil() {
	o.TaxAmount.Set(nil)
}

// UnsetTaxAmount ensures that no value is present for TaxAmount, not even an explicit nil
func (o *UnpostedProcurementTaxableLevel) UnsetTaxAmount() {
	o.TaxAmount.Unset()
}

// GetInfo returns the Info field value if set, zero value otherwise.
func (o *UnpostedProcurementTaxableLevel) GetInfo() map[string]string {
	if o == nil || IsNil(o.Info) {
		var ret map[string]string
		return ret
	}
	return *o.Info
}

// GetInfoOk returns a tuple with the Info field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UnpostedProcurementTaxableLevel) GetInfoOk() (*map[string]string, bool) {
	if o == nil || IsNil(o.Info) {
		return nil, false
	}
	return o.Info, true
}

// HasInfo returns a boolean if a field has been set.
func (o *UnpostedProcurementTaxableLevel) HasInfo() bool {
	if o != nil && !IsNil(o.Info) {
		return true
	}

	return false
}

// SetInfo gets a reference to the given map[string]string and assigns it to the Info field.
func (o *UnpostedProcurementTaxableLevel) SetInfo(v map[string]string) {
	o.Info = &v
}

func (o UnpostedProcurementTaxableLevel) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UnpostedProcurementTaxableLevel) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.TaxLevel) {
		toSerialize["taxLevel"] = o.TaxLevel
	}
	if !IsNil(o.TaxCodeXref) {
		toSerialize["taxCodeXref"] = o.TaxCodeXref
	}
	if o.TaxAmount.IsSet() {
		toSerialize["taxAmount"] = o.TaxAmount.Get()
	}
	if !IsNil(o.Info) {
		toSerialize["_info"] = o.Info
	}
	return toSerialize, nil
}

type NullableUnpostedProcurementTaxableLevel struct {
	value *UnpostedProcurementTaxableLevel
	isSet bool
}

func (v NullableUnpostedProcurementTaxableLevel) Get() *UnpostedProcurementTaxableLevel {
	return v.value
}

func (v *NullableUnpostedProcurementTaxableLevel) Set(val *UnpostedProcurementTaxableLevel) {
	v.value = val
	v.isSet = true
}

func (v NullableUnpostedProcurementTaxableLevel) IsSet() bool {
	return v.isSet
}

func (v *NullableUnpostedProcurementTaxableLevel) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUnpostedProcurementTaxableLevel(val *UnpostedProcurementTaxableLevel) *NullableUnpostedProcurementTaxableLevel {
	return &NullableUnpostedProcurementTaxableLevel{value: val, isSet: true}
}

func (v NullableUnpostedProcurementTaxableLevel) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUnpostedProcurementTaxableLevel) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


